...
if __name__ == '__main__':
    ...
    gpu = int(input('¿Quieres usar ejecución con CPU o GPU? (0 - CPU, 1 - GPU) : '))
    if gpu != 0 and gpu != 1:
    ...
    if personalizado != 0 and personalizado != 1:
        ...
        version=int(input('¿Usar la versión clásica o semiclásica? (0 - Clásica, 1 - Semiclásica): '))
        ...
        if version == 0:
            ...
            qubits = 2*n + 3
            mejora=int(input('¿Usar la versión optimizada para la construcción del circuito? (0 - No, 1 - Sí): '))
            if mejora != 0 and mejora != 1:
                print('Selecciona una opción correcta')
                exit()
        else:
            print('Selecciona una opción correcta')
    ...
    start = time.time()
    """ Print info to user """
    print('Ejecutando {0} veces para N={1} y a={2}\n'.format(number_shots,N,a))
