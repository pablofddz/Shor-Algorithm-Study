...
if __name__ == '__main__':
    ...
    result = Sampler(session=session).run(transpiled_circuit, shots=number_shots).result().quasi_dists[0]
    ...
    counts_result = [key / (2 ** (2*4)) for key, value in result.items()]
    print(counts_result)

    prob=0
    for phase in counts_result:
